  ...
  try {
    ...
    try {
      ...
      if (registrantEntity && registrantEntity.vcardArray) {
        const adr = registrantEntity.vcardArray[1]?.find(v => v[0] === "adr");
        if (adr && adr[3] && adr[3].country_name) {
          country = adr[3].country_name;
        }
      }
    } catch (e) {
      console.log("Could not extract country");
    }
    ...
    // Return null data if lookup fails
    return {
      createdDate: null,
      expiresDate: null,
      registrar: null,
...
chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
  ...
  // Get domain WHOIS info (ASYNC)
  if (msg.type === "GET_DOMAIN_INFO") {
    fetchWhoisData(msg.domain)
      .then(data => {
